...
//  【山口さん専用スプレッドシート版】
//
//  福山・古川・清田の3人は「稼働費計算_GAS版」1冊に同居しているが、
//  山口さんは現場が福岡・佐賀方面で他の3人と別エリアのため、
//  もともと別のスプレッドシート（山口_稼働費計算_GAS版）に
//  自分の実績・現場一覧（稼働場所タブ）を持っている。
//  データを移さず、この専用ブックに「同じ仕組み」を足すためのコピー。
//  中身は commute-expense-sheet.gs と同じで、STAFF_HOME を
//  山口さんだけに絞ってあるだけ（他は完全に同一）。
//
//  【セットアップ手順】
//  1. 「山口_稼働費計算_GAS版」を開く
//  2. 拡張機能 → Apps Script → 既存のコードを全て削除してこれを貼り付けて保存
...
//     　デプロイ後に出る /exec のURLを控えて、ポータル担当者に共有する
//  6. 以降は月を変えたら「🗓️ 月を更新」を実行
//
//  【日々の使い方】
//  ・C列（稼働場所）をプルダウンで選択
//    → I列（高速 有/無）をプルダウンで選択
...
const CONFIG = {
  ...
  // このブックは山口さん専用なので、他の3人は含めない
  // （含めても実害はないが、シートが無い名前を誤って選べてしまうのを避ける）
  STAFF_HOME: {
    '山口真人': '山口自宅',
  },
